...
SAFE="${SAFE_ADDRESS:-0x31837999D9E463B2EB4327CEb4BD7CCa2a500480}"
...
assert_eq "safeThreshold" "$(read_uint "$SAFE" 'getThreshold()(uint256)')" "1"

market_owner="$(cast call "$MARKET" 'owner()(address)' --rpc-url "$RPC_URL" | tr '[:upper:]' '[:lower:]')"
assert_eq "marketOwner" "$market_owner" "$(tr '[:upper:]' '[:lower:]' <<< "$SAFE")"
...
grep -q "$SAFE" web/src/config/protocol.ts
